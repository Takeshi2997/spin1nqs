...
export compute_SR_update, compute_jacobian, SR_update
...
SR法
"""
function SR_update(O_mean, OO_mean, OE_mean, E_mean, epoch, epsilon=0.01f0, epsilon2=0.f0, decay=0.999f0, lambda_min=1f-6)
    # S行列とRベクトルの構築
    S = real.(OO_mean .- conj.(O_mean) * transpose(O_mean))          # S_kl
    R = real.(OE_mean .- conj.(O_mean) .* E_mean)
